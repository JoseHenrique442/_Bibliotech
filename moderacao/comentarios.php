...
<head>
    ...
    <link rel="stylesheet" href="comentarios.css" type="text/css">
    <link rel="stylesheet" href="../global.css" type="text/css">
    <title>Moderação de Comentários | Bibliotech</title>
</head>
<body>
    <!-- MENU LATERAL -->
    <nav>
    ...
    <main id="home">
        ...
        <div class="container">
            ...
            <?php
                ...
                $sql = mysqli_query($conexao, "Select DESCRICAO from comentarios where APROVADO = 0");
                $sqln = mysqli_query($conexao, "Select AUTOR from comentarios where APROVADO = 0");
                $bah = mysqli_query($conexao, "Select TITULO_OBRA from comentarios where APROVADO = 0");
                while($row = mysqli_fetch_assoc($sql)){
                    $rows = mysqli_fetch_assoc($sqln);
                    $ro = mysqli_fetch_assoc($bah);
                    echo '
                        <div class="comment__wrapper">
                            <h2>Titulo da Obra: '.$ro['TITULO_OBRA'].'</h2>
                            <h2>Autor: '.$rows['AUTOR'].'</h2>
                            <form method="POST" action="./apro.php">
                                <div class="comment__box">
                                <div class="user__img">
                                    <img src="../img/user-1.jpg" alt="FOTO DE PERFIL">
                                </div>
                                <div class="comment__input">
                                    <textarea value="'.$row['DESCRICAO'].'"name="DESC" id="" maxlength="500" required readonly>'.$row['DESCRICAO'].'</textarea>
                                </div>
                                </div>
                                <div class="buttons">                
                                    <button id="aceitar" name="Sim" value="aprovado">Aceitar</button>
                                    <button id="recusar" name="Sim" value="recusado">Recusar</button>
                                </div>
                            </form>
                        </div>';
                }
